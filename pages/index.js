...
import Index from '../components/connectors/Index';
...
export default class extends React.Component {
  ...
  static async getInitialProps({ path, query, req, res }) {
    ...
    ]).then(([{ cardsCategories }, { categoryCards }, { featuredCards }]) => {
  ...
  componentDidMount() {
    if (window.location.hash) {
      ...
      if (!(url.indexOf('about_us') || url.indexOf('public_cards') || url.indexOf('what_is_wishyoo'))) {
        window.location = `${window.location.hash.replace('#', '')}${window.location.search}`;
      }
    }
  }
